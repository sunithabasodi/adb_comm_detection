...
class ViewerApp(tk.Tk):
    ...
    def __init__(self, graph, title, **kwargs):
        ...
        self.canvas = GraphCanvas(graph, width=100, height=100, **kwargs)
        self.canvas.grid(row=0, column=0, rowspan=bottom_row+2, sticky='NESW')
        #self.canvas.place( anchor=tk.CENTER)
        ...
        tk.Button(self, text='Show Original', command=self.showOriginal
    ...
    def showOriginal(self):
        """Show original graph"""
        home_nodes =self.canvas.dataG.nodes()
        self.canvas.plot(home_nodes, levels=self.level)
