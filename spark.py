#!/usr/bin/env python
import networkx as nx
import numpy as np
import glob
import os, os.path
import math
import matplotlib.pyplot as plt

class UnDirectedEgoNetworkLoader :
    def __init__(self, dataset):
        self.dataset_name=dataset
        self.pathhack = os.getcwd() + "/Dataset/" +self.dataset_name
        self.feat_file_name = "%s/feature_map.txt" % (self.pathhack,)
        self.feature_index = {}  #numeric index to name
        self.inverted_feature_index = {} #name to numeric index
        self.network = nx.Graph()
        self.ego_nodes = []

    def parse_featname_line(self, line):
        line = line[(line.find(' '))+1:]  # chop first field
        split = line.split(';')
        name = ';'.join(split[:-1]) # feature name
        index = int(split[-1].split(" ")[-1]) #feature index
        return index, name

    def load_features(self):
        # may need to build the index first
        if not os.path.exists(self.feat_file_name):
            feat_index = {}
            # build the index from *.featnames files
            featname_files = glob.iglob("%s*.featnames" % (self.pathhack,))
            for featname_file_name in featname_files:
                featname_file = open(featname_file_name, 'r')
                for line in featname_file:
                    # example line:
                    # 0 birthday;anonymized feature 376
                    index, name = self.parse_featname_line(line)
                    feat_index[index] = name
                featname_file.close()
            keys = feat_index.keys()
            keys.sort()
            out = open(self.feat_file_name,'w')
            for key in keys:
                out.write("%d %s\n" % (key, feat_index[key]))
            out.close()

        # index built, read it in (even if we just built it by scanning)
        #global feature_index
        #global inverted_feature_index
        index_file = open(self.feat_file_name,'r')
        for line in index_file:
            split = line.strip().split(' ')
            key = int(split[0])
            val = split[1]
            self.feature_index[key] = val
        index_file.close()

        for key in self.feature_index.keys():
            val = self.feature_index[key]
            self.inverted_feature_index[val] = key

    def load_nodes(self):
        assert len(self.feature_index) > 0, "call load_features() first"
        #global network
        #global ego_nodes

        # get all the node ids by looking at the files
        ego_nodes = [int(x.split("/")[-1].split('.')[0]) for x in glob.glob("%s/*.featnames" % (self.pathhack,))]
        node_ids = ego_nodes

        # parse each node
        for node_id in node_ids:
            featname_file = open("%s/%d.featnames" % (self.pathhack,node_id), 'r')
            feat_file     = open("%s/%d.feat"      % (self.pathhack,node_id), 'r')
            egofeat_file  = open("%s/%d.egofeat"   % (self.pathhack,node_id), 'r')
            edge_file     = open("%s/%d.edges"     % (self.pathhack,node_id), 'r')

            # parse ego node
            self.network.add_node(node_id)
            # 0 1 0 0 0 ...
            ego_features = [int(x) for x in egofeat_file.readline().split(' ')]
            i = 0
            self.network.node[node_id]['features'] = np.zeros(len(self.feature_index))
            for line in featname_file:
                key, val = self.parse_featname_line(line)
                self.network.node[node_id]['features'][key] = ego_features[i] + 1
                i += 1

            # parse neighboring nodes
            for line in feat_file:
                featname_file.seek(0)
                split = [int(x) for x in line.split(' ')]
                node_id = split[0]
                features = split[1:]
                self.network.add_node(node_id)
                self.network.node[node_id]['features'] = np.zeros(len(self.feature_index))
                i = 0
                for line in featname_file:
                    key, val = self.parse_featname_line(line)
                    self.network.node[node_id]['features'][key] = features[i]
                    i += 1

            featname_file.close()
            feat_file.close()
            egofeat_file.close()
            edge_file.close()

    def load_edges(self):
        #global network
        assert self.network.order() > 0, "call load_nodes() first"
        edge_file = open("%s/facebook_combined.txt" % (self.pathhack,),"r")
        for line in edge_file:
            # nodefrom nodeto
            split = [int(x) for x in line.split(" ")]
            node_from = split[0]
            node_to = split[1]
            self.network.add_edge(node_from, node_to)

    def load_network(self):
        """
        Load the network.  After calling this function, facebook.network points to a networkx object for the facebook data.

        """
        self.load_features()
        self.load_nodes()
        self.load_edges()

    def feature_matrix(self):
        n_nodes = self.network.number_of_nodes()
        n_features = len(self.feature_index)

        X = np.zeros((n_nodes, n_features))
        for i,node in enumerate(self.network.nodes()):
            X[i,:] = self.network.node[node]['features']

        return X

    def universal_feature(self, feature_index):
        """
        Does every node have this feature?

        """
        return len([x for x in self.network.nodes_iter()
                    if self.network.node[x]['feautures'][feature_index] > 0]) \
               // self.network.order() == 1

class DirectedEgoNetworkLoader :
    def __init__(self, dataset):
        self.dataset_name=dataset
        self.pathhack = os.getcwd() + "/Dataset/" +self.dataset_name
        self.edge_list_file_name = "%s/%s_combined.txt" % (self.pathhack,self.dataset_name)
        self.network = None

    def load_network(self):
        """
        Load the network.  After calling this function, facebook.network points to a networkx object for the twitter data.

        """
        lines = [line.rstrip('\n') for line in open(self.edge_list_file_name)]
        self.network = nx.parse_edgelist(lines, nodetype = int)
        print self.network.order()

def twitter_test():
    print "Running tests."
    print "Loading network..."
    und = DirectedEgoNetworkLoader("twitter")
    und.load_network()
    network = und.network
    print "done."
    #nx.write_gexf(network, "facebook.gexf")
    failures = 0
    def test(actual, expected, test_name):
        global failures  #lol python scope
        try:
            print "testing %s..." % (test_name,)
            assert actual == expected, "%s failed (%s != %s)!" % (test_name,actual, expected)
            print "%s passed (%s == %s)." % (test_name,actual,expected)
        except AssertionError as e:
            print e
            failures += 1
    #Number of nodes
    test(network.order(), 81306, "order")
    #Number of Edges
    #test(network.size(), 1768149, "size") # Use this test if directed
    test(network.size(), 1342310, "size") #Less than original due to considering them as undirected;
    #Average clustering coefficient
    test(round(nx.average_clustering(network),4), 0.5653, "clustering")
    #nx.draw(network)
    #plt.show()
    print "%d tests failed." % (failures,)

def facebook_test():
    print "Running tests."
    print "Loading network..."
    und = UnDirectedEgoNetworkLoader("facebook")
    und.load_network()
    network = und.network
    print "done."
    #nx.write_gexf(network, "facebook.gexf")
    failures = 0
    def test(actual, expected, test_name):
        global failures  #lol python scope
        try:
            print "testing %s..." % (test_name,)
            assert actual == expected, "%s failed (%s != %s)!" % (test_name,actual, expected)
            print "%s passed (%s == %s)." % (test_name,actual,expected)
        except AssertionError as e:
            print e
            failures += 1
    #Number of nodes
    test(network.order(), 4039, "order")
    #Number of Edges
    test(network.size(), 88234, "size")
    #Average clustering coefficient
    test(round(nx.average_clustering(network),4), 0.6055, "clustering")
    #nx.draw(network)
    #plt.show()
    print "%d tests failed." % (failures,)

if __name__ == '__main__':
    facebook_test()
    #twitter_test()