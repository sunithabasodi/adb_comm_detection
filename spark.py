...
pathhack = os.path.dirname(os.path.realpath(__file__)) + "/Dataset/facebook"
...
def load_features():
    ...
    if not os.path.exists(feat_file_name):
        ...
        featname_files = glob.iglob("%s*.featnames" % (pathhack,))
...
def load_nodes():
    ...
    ego_nodes = [int(x.split("/")[-1].split('.')[0]) for x in glob.glob("%s/*.featnames" % (pathhack,))]
    node_ids = ego_nodes

    # parse each node
    for node_id in node_ids:
        featname_file = open("%s/%d.featnames" % (pathhack,node_id), 'r')
        feat_file     = open("%s/%d.feat"      % (pathhack,node_id), 'r')
        egofeat_file  = open("%s/%d.egofeat"   % (pathhack,node_id), 'r')
        edge_file     = open("%s/%d.edges"     % (pathhack,node_id), 'r')
